...
let projectLeaderGroupFeedbackForumConnection = new WebSocket(`ws://localhost:8080/groups/feedback?project=${projectID}&group=${groupID}`);

projectLeaderGroupFeedbackForumConnection.onopen = (event) => {
    console.log(event.data);
}

projectLeaderGroupFeedbackForumConnection.onclose = (event) => {
    console.log(event.data);
}

projectLeaderGroupFeedbackForumConnection.onerror = (event) => {
    console.error(event.data);
}

projectLeaderGroupFeedbackForumConnection.onmessage = (event) => {
...
async function onStartUp() {
    ...
    let url = "http://localhost/public/project/leader/group/feedback/messages";
    try {
...
async function sendMessages(msg) {
    ...
    projectLeaderGroupFeedbackForumConnection.send(JSON.stringify(msgObj));
    console.log(msgObj);
