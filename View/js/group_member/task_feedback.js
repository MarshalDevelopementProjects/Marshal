...
let groupMemberGroupFeedbackForumConnection = new WebSocket(`ws://localhost:8080/tasks/feedback?project=${projectID}&task=${taskID}`);

groupMemberGroupFeedbackForumConnection.onopen = (event) => {
    console.log(event.data);
}

groupMemberGroupFeedbackForumConnection.onclose = (event) => {
    console.log(event.data);
}

groupMemberGroupFeedbackForumConnection.onerror = (event) => {
    console.error(event.data);
}

groupMemberGroupFeedbackForumConnection.onmessage = (event) => {
...
async function onStartUp() {
    ...
    let url = `http://localhost/public/group/member/task/feedback/messages?task_id=${taskID}`;
    try {
        ...
        console.error(error);
        groupMemberGroupFeedbackForumConnection.close();
    }
}
...
async function sendMessages(msg) {
    ...
    groupMemberGroupFeedbackForumConnection.send(JSON.stringify(msgObj));
    console.log(msgObj);

    // TODO: SEND THE MESSAGE TO THE APPROPRIATE END POINT(ASYNC)
    let url = "http://localhost/public/group/member/task/feedback/messages";
    let requestBody = {
        task_id: taskID,
