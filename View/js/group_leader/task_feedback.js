...
let groupLeaderGroupTaskFeedbackForumConnection = new WebSocket(`ws://localhost:8080/tasks/feedback?project=${projectID}&task=${taskID}`);

groupLeaderGroupTaskFeedbackForumConnection.onopen = (event) => {
    console.log(event.data);
}

groupLeaderGroupTaskFeedbackForumConnection.onclose = (event) => {
    console.log(event.data);
}

groupLeaderGroupTaskFeedbackForumConnection.onerror = (event) => {
    console.error(event.data);
}

groupLeaderGroupTaskFeedbackForumConnection.onmessage = (event) => {
...
async function onStartUp() {
    ...
    let url = `http://localhost/public/group/leader/task/feedback/messages?task_id=${taskID}`;
    try {
        ...
        console.error(error);
        groupLeaderGroupTaskFeedbackForumConnection.close();
    }
}
...
async function sendMessages(msg) {
    ...
    groupLeaderGroupTaskFeedbackForumConnection.send(JSON.stringify(msgObj));
    console.log(msgObj);

    // TODO: SEND THE MESSAGE TO THE APPROPRIATE END POINT(ASYNC)
    let url = "http://localhost/public/group/leader/task/feedback/messages";
    let requestBody = {
        task_id: taskID,
